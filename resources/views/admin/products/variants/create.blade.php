...
@section('content')
    ...
    <form action="{{ route('admin.products.variants.store', $product) }}" method="POST" enctype="multipart/form-data" id="variantForm">
        @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-400 mb-1.5">
                                SKU / Kode Produk <span class="text-red-400">*</span>
                            </label>
                            <div class="flex gap-2">
                                <input type="text" name="sku" id="skuInput"
                                       value="{{ old('sku') }}" required
                                       placeholder="Contoh: KAOS-MERAH-XL"
                                       class="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3.5 py-2.5 text-sm text-white font-mono placeholder-gray-600
                                ...
                                <button type="button" id="genSkuBtn"
                                        class="shrink-0 text-xs text-gray-400 hover:text-white border border-gray-700 hover:border-gray-500 px-3 py-2.5 rounded-lg transition-colors">
                                    Auto-generate
                                </button>
                            </div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <div id="mainColorSwatch"
                                         ...
                                         style="{{ old('color') ? 'background:'.old('color') : 'background:#374151' }}">
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="price_override"
                                       value="{{ old('price_override') }}" min="0" step="100"
                                       placeholder="Kosong = ikut induk"
                                       class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3.5 py-2.5 text-sm text-white placeholder-gray-600
                                              focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-colors">
                                <p class="text-xs text-gray-600 mt-1">Harga induk: {{ $product->formattedPrice() }}</p>
                            </div>
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p class="text-xs text-gray-600 mt-0.5">Tulis label sendiri. Contoh: Ukuran, Bahan, dll.</p>
                        </div>
                        <button type="button" id="addAttrBtn"
                                class="flex items-center gap-1.5 text-xs text-violet-400 hover:text-violet-300 border border-violet-500/30 hover:border-violet-500/60 px-3 py-1.5 rounded-lg transition-colors">
...
@push('scripts')
<script>
let attrIdx = {{ count(old('attributes', [])) }};
let specIdx = {{ count(old('specs', [])) }};

// ── SKU auto-generate ──────────────────────────────────────────────────────
const slugBase = @json(\Illuminate\Support\Str::upper(\Illuminate\Support\Str::slug($product->name, '-')));
document.getElementById('genSkuBtn').addEventListener('click', () => {
    const rand = Math.random().toString(36).substring(2, 6).toUpperCase();
    document.getElementById('skuInput').value = slugBase + '-' + rand;
});
...
function drawImageToCanvas(blobUrl) {
    const img = new Image();
    // Tidak perlu crossOrigin — blob:// URL lokal, tidak ada CORS
    img.onload = () => {
